...
export const useTodoStore = defineStore('todo', {
    ...
    getters: {
        ...
        async getUserTodoList() {
            if (this.isLoggedIn) {
                ...
                if (docSnap.exists() && docSnap.data().todos != null) {
                    this.todos = docSnap.data().items;
                    return;
                }
    ...
    actions: {
        ...
        updateCompleted(id) {
            ...
            if (existingItem) {
                this.completedItems.splice(
                    this.completedItems.findIndex((element) => element.id === id),
                    1
                );
            ...
            this.activeItems = this.allItems.filter(
                (element) => !this.completedItems.includes(element)
            );
        },
        clearCompleted() {
